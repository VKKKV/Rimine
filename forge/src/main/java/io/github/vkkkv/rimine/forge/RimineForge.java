...
public class RimineForge {
        ...
        event.getDispatcher().register(
            LiteralArgumentBuilder.<CommandSourceStack>literal("rimine")
                .then(LiteralArgumentBuilder.<CommandSourceStack>literal("reload").executes(context -> {
                    Path configPath = Minecraft.getInstance().gameDirectory.toPath().resolve("config").resolve("rimine.json");
                    RimeInputHandler.reload(configPath);
                    context.getSource().sendFeedback(Component.literal("Rimine configuration reloaded."), false);
                    return 1;
                }))
        );
    }
    ...
    public void onRenderGui(RenderGuiOverlayEvent.Post event) {
        ...
        int height = (candidates.size() + (data.composition() != null ? 1 : 0) + (data.isSwitcher() ? 1 : 0) + 1) * lineHeight + 4;

        // Draw background using config
        event.getGuiGraphics().fill(x - 2, y - 2, x + width, y + height - 2, config.getResolvedBgColor());

        int currentY = y;
        if (data.isSwitcher()) {
            event.getGuiGraphics().drawString(mc.font, "§6Schema Selection:", x, currentY, 0xFFFFFF, false);
            currentY += lineHeight;
        }
        ...
        for (int i = 0; i < candidates.size(); i++) {
            ...
            event.getGuiGraphics().drawString(mc.font, prefix + (i + 1) + ". " + candidates.get(i), x, currentY, color, false);
            currentY += lineHeight;
        }
