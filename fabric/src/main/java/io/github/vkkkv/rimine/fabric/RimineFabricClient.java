...
public class RimineFabricClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> {
            dispatcher.register(ClientCommandManager.literal("rimine")
                .then(ClientCommandManager.literal("reload").executes(context -> {
                    ...
                    context.getSource().sendFeedback(Component.literal("Rimine configuration reloaded."));
                    return 1;
                })));
        });
        ...
        HudRenderCallback.EVENT.register((guiGraphics, tickDelta) -> {
            ...
            int currentY = y;
            if (data.isSwitcher()) {
                guiGraphics.drawString(client.font, "§6Schema Selection:", x, currentY, 0xFFFFFF, false);
                currentY += lineHeight;
            }
            ...
            for (int i = 0; i < candidates.size(); i++) {
                ...
                guiGraphics.drawString(client.font, prefix + (i + 1) + ". " + candidates.get(i), x, currentY, color, false);
                currentY += lineHeight;
            }
